...
void CHyprRenderer::renderMonitor(CMonitor* pMonitor) {
    ...
    pMonitor->output->state->presentationMode =
...
bool CHyprRenderer::applyMonitorRule(CMonitor* pMonitor, SMonitorRule* pMonitorRule, bool force) {
    ...
    if (RULE->resolution != Vector2D() && RULE->resolution != Vector2D(-1, -1) && RULE->resolution != Vector2D(-1, -2)) {
        if (!pMonitor->output->modes.empty() && RULE->drmMode.type != DRM_MODE_TYPE_USERDEF) {
            ...
            for (auto& mode : pMonitor->output->modes) {
                ...
                if (DELTALESSTHAN(mode->pixelSize.x, RULE->resolution.x, 1) && DELTALESSTHAN(mode->pixelSize.y, RULE->resolution.y, 1) &&
                    ...
                    pMonitor->output->state->mode = mode;
            ...
            if (!found) {
                pMonitor->output->state->customMode = makeShared<Aquamarine::SOutputMode>(Aquamarine::SOutputMode{.pixelSize = RULE->resolution, .refreshRate = WLRREFRESHRATE});
                ...
                if (!pMonitor->state.test()) {
                    ...
                    // Preferred is valid
                    pMonitor->output->state->mode = PREFERREDMODE;
        ...
        if (!pMonitor->output->modes.empty()) {
            ...
            if (!success) {
                if (pMonitor->output->state->mode)
                    Debug::log(LOG, "Monitor {}: REJECTED mode: {:X0}@{:2f}! Falling back to preferred: {}x{}@{:2f}", pMonitor->output->name, RULE->resolution,
                               (float)RULE->refreshRate, pMonitor->output->state->mode->pixelSize.x, pMonitor->output->state->mode->pixelSize.y,
                               pMonitor->output->state->mode->refreshRate / 1000.f);
    ...
    pMonitor->vrrActive = pMonitor->output->state->adaptiveSync // disabled here, will be tested in CConfigManager::ensureVRR()
    ...
    for (auto& fmt : formats[(int)!RULE->enable10bit]) {
        pMonitor->output->state->drmFormat = fmt.second;
...
void CHyprRenderer::endRender() {
    ...
    if (m_eRenderMode == RENDER_MODE_NORMAL)
        PMONITOR->output->state->buffer = m_pCurrentBuffer;
